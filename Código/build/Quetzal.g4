...
scaleImg: TK_SCALE_IMAGE SYM_PAREN_OPEN CTE_TAG{stack.Types.append("tag")}{stack.addConstant($CTE_TAG.text)}  SYM_COMMA TYPE_ID SYM_COMMA expression SYM_COMMA expression SYM_PAREN_CLOSE{stack.scaleImg($TYPE_ID.text)};
getColorPalette: TK_GET_COLOR_PALETTE SYM_PAREN_OPEN TYPE_ID{stack.Opd.append($TYPE_ID.text)} SYM_COMMA TYPE_ID SYM_COMMA expression SYM_PAREN_CLOSE {stack.getColorPalette($TYPE_ID.text)}; 
